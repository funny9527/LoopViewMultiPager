...
public class ScrollPanel extends ViewGroup {
	...
	public ScrollPanel(Context context, AttributeSet attrs) {
		...
		paddingHron = a.getDimensionPixelSize(R.styleable.ScrollPanel_paddingHron, 0);
		a.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Log.v(TAG, "=============== " + MeasureSpec.getSize(widthMeasureSpec)
		    + "      " + paddingHron);

        screenWidth = MeasureSpec.getSize(widthMeasureSpec) - 2 * paddingHron;
        
        for (int i = 0; i < childCount; i++) {
            View childView = getChildAt(i);
            widthMeasureSpec =  MeasureSpec.makeMeasureSpec(screenWidth, widthMode);
            measureChild(childView, widthMeasureSpec, heightMeasureSpec);
            pageWidth = childView.getMeasuredWidth();
        }

		Log.v(TAG, "screenWidth = " + screenWidth + "    pageWidth = " + pageWidth);
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (changed) {
            ...
            int cleft = (screenWidth - pageWidth) / 2 + paddingHron;
            int cright = 0;
            for (int i = 0; i < childCount; i++) {
                View childView = getChildAt(i);
                cright = cleft + childView.getMeasuredWidth();
                childView.layout(cleft, 0, cright, childView.getMeasuredHeight());
                cleft = cright + 2 * paddingHron;
            }
            
            if (childCount > 0) {
            	mChildWidth = getChildAt(0).getWidth() + 2 * paddingHron;
    ...
    protected void dispatchDraw(Canvas canvas) {
		...
		if (isLoopEnabled && mPageCount > 2) {
			if (right >= mPageCount - 1) {
				if (right == mPageCount - 1) {
					drawChild(canvas, getChildAt(left), getDrawingTime());
					drawChild(canvas, getChildAt(right), getDrawingTime());
					drawChild(canvas, getChildAt(left + 1), getDrawingTime());
				...
				if (mUnboundedScrollX >= 0) {
					canvas.translate(-offset, 0);
					drawChild(canvas, getChildAt(mPageCount - 1), getDrawingTime());
					canvas.translate(offset, 0);
					
					drawChild(canvas, getChildAt(2), getDrawingTime());
					drawChild(canvas, getChildAt(1), getDrawingTime());
					drawChild(canvas, getChildAt(0), getDrawingTime());
